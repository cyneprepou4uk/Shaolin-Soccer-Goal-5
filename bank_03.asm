...
_loc_03_818F:		; бесконечный цикл, видимо не использутся, удалить
	LDA #$00
	JMP _loc_03_818F
...
bra_03_8503:
	...
	CMP #$C0
	BEQ bra_03_8511
	LDA номер_анимации,Y	; баг. в этом месте надо делать LDY игрока с мячом, потому что выше Y перезапишется поведем
	BPL bra_03_851F
bra_03_8511:
...
@RTS:
	RTS
@дальше:
	LDA номер_погодного_эффекта		; не нужно повторно загружать погоду, удалить
	AND #$7F
	BEQ bra_03_BB1E
	CMP #ПОГОДА_ВЕТЕР
	BEQ bra_03_BB1E
