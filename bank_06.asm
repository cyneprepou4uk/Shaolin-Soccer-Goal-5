...
table_06_8CAF_8CE2:
	...
	LDA #$05
	JSR _loc_06_949C_чеканка_отскок_мяча_от_игрока
bra_06_8CF4:
	LDA скорость_Z_hi,X
	BMI bra_06_8D30
	CLC
...
bra_06_90B3:
	...
	DEC гравитация_lo,X		; 60fps временное решение, проблема в том, что мяч слишком часто ударяется обземлю
							; отсюда и постоянный звук, а еще частая смена анимации мяча
...
; после этого игроку присваивается владение мячом, и он может нажать ударить и попадет по мячу независимо от его местоположения
	...
	CLC
	LDA скорость_Z_lo,Y
	ADC #$80		; 60фпс. когда игрок принимает мяч на грудь или голову, при условии что до этого не владел мячом
					; тут внимательнее, скорость игрока прибавляется к скорости мяча
					; при оригинальных 80 выглядит как в оригинале, возможно и не надо уменьшать
	STA скорость_Z_lo,X
	LDA скорость_Z_hi,Y
	ADC #$00
	STA скорость_Z_hi,X
...
bra_06_91DA:
	...
	JSR _подправить_скорости_1C_1D		; 60fps
	SEC
	LDA #$55		; 60fps скорость Z
	SBC $1C
	STA скорость_Z_lo,X
	LDA #$01		; 60fps скорость Z
	SBC $1D
	STA скорость_Z_hi,X
	LDA #$00
	STA гравитация_hi,X
...
_loc_06_949C_чеканка_отскок_мяча_от_игрока:
	ASL
	ASL
	TAY
	LDA таймер_мокрого_мяча
...
bra_06_954E:
	...
	EOR #$FF
	BEQ bra_06_956B
	JSR _подправить_скорости_30_31		; 60fps
	SEC
	LDA скорость_X_lo,X
	SBC $30
	STA скорость_X_lo,X
...
bra_06_959C:
	...
	EOR #$FF
	BEQ bra_06_95B9
	JSR _подправить_скорости_32_33		; 60fps
	SEC
	LDA скорость_Y_lo,X
	SBC $32
	STA скорость_Y_lo,X
...
_подправить_скорости_30_31:		; 60fps корректировка скоростей, вероятно при скольжении мяча по полю
	LDA $30
	STA $0192
	LDA $31
	STA $0193
...
_подправить_скорости_32_33:		; 60fps корректировка скоростей, вероятно при скольжении мяча по полю
	LDA $32
	STA $0192
	LDA $33
	STA $0193
...
bra_06_9787:
	...
	BNE bra_06_97B0
bra_06_97AD:
	JSR _b07_D7F3_вычисление_следов_на_поле
bra_06_97B0:
	RTS
...
; X - номер нарушителя, Y - номер отпизженного
	...
	INY
@запись_позиции:
	LDA table_06_9E6D,Y		; bzk 021 удалено BMI после чтения таблицы
...
table_06_9E6D:		; bzk 021 удален последний байт 03, так как по коду Y никогда не будет 03
.byte $01,$03,$02
...
.byte $F3,$03		; DVA 03 поделить на 3
	CLC
	ADC table_остаток_деления_A4D8,Y
	STA младший_байт
	LDA старший_байт
	ADC #$00
	STA старший_байт
