...
_loc_04_801B:
	JMP _loc_04_BDE3
.export _loc_04_801E_проверка_координат_для_возможности_сделать_супер
_loc_04_801E_проверка_координат_для_возможности_сделать_супер:
	JMP _loc_04_BA1B_проверка_координат_для_возможности_сделать_супер
.export _loc_04_8021
_loc_04_8021:
	JMP _loc_04_BB1E
.export _loc_04_8024
...
_подправить_скр_ветр:		; 60fps
	LDA скорость_X_lo_ветер
	STA $0192
	LDA скорость_X_hi_ветер
	STA $0193
	LDA #$33
	...
	LDA $0192
	STA скорость_X_lo_ветер
	LDA скорость_Y_lo_ветер
	STA $0192
	LDA скорость_Y_hi_ветер
	STA $0193
	LDA #$33
	JSR _b07_деление_и_умножение_скоростей_16bit_на_4
	LDA $0193
...
_loc_04_B3EA:
	...
	STA $2D
bra_04_B3FB:
	JSR _подправить_скорости_2C_2D
	CLC
	LDA скорость_X_lo,X
	ADC $2C
	STA скорость_X_lo,X
...
_loc_04_B423:
	...
	STA $2F
bra_04_B434:
	JSR _подправить_скорости_2E_2F
	CLC
	LDA скорость_Y_lo,X
	ADC $2E
	STA скорость_Y_lo,X
...
_подправить_скорости_2C_2D:		; 60fps корректировка скорости смещения от смерча
	LDA $2C
	STA $0192
	LDA $2D
	STA $0193
...
_подправить_скорости_2E_2F:		; 60fps корректировка скорости смещения от смерча
	LDA $2E
	STA $0192
	LDA $2F
	STA $0193
...
bra_04_B4DE:
	...
	STA $1E
	LDA сумма_удержанных_кнопок,X
	STA $1F
	LDA сумма_нажатых_кнопок_А_и_Б,X
	STA $20
...
_loc_04_BA1B_проверка_координат_для_возможности_сделать_супер:
	LDY игрок_с_мячом
	LDA направление_движения,Y
	STA направление_движения,X
...
; сравнивается координата бьющего игрока
; чтобы тот не был слишком далеко или слишком близко к воротам соперника
; чтобы ему разрешили сделать супер
	LDA координата_X_lo,X
	STA $2C
	LDA координата_X_hi,X
	STA $2D
	LDA направление_движения,X
	BPL @игрок_движется_вправо
	CLC
	LDA #$00
	SBC $2C
	STA $2C
	...
	SBC $2D
	STA $2D
@игрок_движется_вправо:
	LDY #$00
@цикл:
	LDA $2D
	CMP table_04_BB0E_старший + 1,Y
	BEQ @равно
	BCC @меньше
	BCS @больше
@равно:
	LDA $2C
	CMP table_04_BB0E_младший,Y
	BCC @меньше
@больше:
	INY
	INY
...
@меньше:
	STY $1C
; сравнивается высота игрока в зависимости от сложности игры
	LDA координата_Z_hi,X
	BNE bra_04_BAD3		; игрок летает в ебенях
	LDY игрок_с_мячом
	LDA номер_движения,Y
	AND #$7F
	CMP #$33
	BEQ @движение_33_или_37
	CMP #$37
	BNE @это_другое_движение
@движение_33_или_37:
	JSR _loc_04_BDC0_получить_Y_для_чтения_таблиц_исходя_из_сложности_и_управления
	LDA координата_Z_lo,X
	CMP table_04_BDD7_высота_удара_с_ноги,Y
	BCC bra_04_BAD3		; игрок недостаточно высоко
	INC $1C
	JMP _loc_04_BA9D
@это_другое_движение:
	JSR _loc_04_BDC0_получить_Y_для_чтения_таблиц_исходя_из_сложности_и_управления
	LDA координата_Z_lo,X
	CMP table_04_BDDD_высота_удара_головой,Y
	BCC bra_04_BAD3		; игрок недостаточно высоко
_loc_04_BA9D:
	LDY $1C
	LDA table_04_BB16,Y
...
@это_не_япония:
	...
	STA $1C
	TAY
	LDA table_04_BD12_сила_супера + 1,Y
	JSR _loc_04_BB02_добавить_A_к_силе_мяча
	LDA table_04_BD12_ном_супера,Y
bra_04_BAC7:
	STA номер_супера
...
_loc_04_BB02_добавить_A_к_силе_мяча:
	CLC
	ADC сила_мяча
	BCC @пропуск
	LDA #$FF
...
table_04_BB0E_младший:
table_04_BB0E_старший:
; координата X
.word $0160
.word $0290
.word $0350
...
_loc_04_BBEC:
	...
	STA разновидность_супера
	TAY
	LDA table_04_BD4A_скорость_X_Y,Y		; 60fps скорости выведены в отдельную таблицу
	BEQ @пропуск
	STA $2D
	LDA #$00
	STA $2C
	JSR _loc_04_802C_вычисление_скорости_удара
@пропуск:
	LDA разновидность_супера
	ASL
	TAY
	LDA table_04_BD4A_скорость_Z,Y
	STA скорость_Z_lo,X
	LDA table_04_BD4A_скорость_Z + 1,Y
	STA скорость_Z_hi,X
	JSR _loc_04_BC62_записать_таймер_супера
...
_loc_04_BC62_записать_таймер_супера:
	LDY разновидность_супера
	LDA table_04_BD5E_таймеры,Y
	BPL @запись_таймера
	JSR _loc_04_BC7F_вычислить_таймер_супера
	LDA $2C
.byte $F2,$04		; DMA 00 60fps длительность супера увеличена в 4 раза
@запись_таймера:
	STA таймер_действия_мяча
	LDA #$00
	STA гравитация_hi,X
	LDA #$08		; 60fps гравитация удара
	STA гравитация_lo,X
	RTS

_loc_04_BC7F_вычислить_таймер_супера:
.SCOPE
младший_байт = $2C
старший_байт = $2D
; походу 02B0 это координаты где нужно приземлиться суперу, типа прицел
; отсюда и вычисление таймера относительно текущих координат мяча
	LDA #$B0
	STA младший_байт
	LDA #$00
	STA старший_байт
...
table_04_BD12_ном_супера:
table_04_BD12_сила_супера:
; номер супера и сила супера
.byte $04,$0F
.byte $16,$0F
.byte $18,$0F
.byte $17,$1E
...
table_04_BD4A_скорость_X_Y:
.byte $08,$08,$08,$10,$08,$00,$00,$08,$04,$04

table_04_BD4A_скорость_Z:	; 60fps скорости некоторых суперов, например красный ближний, уменьшены в 4 раза
.word $0000			; .word $0000 оригинал
.word $FE00			; .word $F800
.word $0040			; .word $0100
.word $0100			; .word $0400
.word $0000			; .word $0000
...
; 60fps ??? таймер полята мяча увеличены в 4 раза (частично)
table_04_BD5E_таймеры:
; обычный удар, красный ближний, красный дальний, ..., звезда
.byte $40,$18,$FF,$7F,$7F,$20,$7F,$FF,$7F,$00,$00
;     $10,$18,$FF,$7F,$7F,$20,$20,$FF,$7F,$00,$00 - оригинал

table_04_BD69:		; читается из 2х мест
; первый байт - базовая разновидность супера, с помощью него читается другая таблица
; второй байт - вариация супера, который получается когда предыдущий супер ударяется об землю
.byte $00,$00
.byte $01,$83
.byte $02,$83
.byte $03,$00
...
_loc_04_BDC0_получить_Y_для_чтения_таблиц_исходя_из_сложности_и_управления:
; Y может получиться от 00 до 06
	LDY игрок_с_мячом
	...
	AND #$03
	ADC #$03		; добавление +03 к сложности
	BNE @скопировать_в_Y
@игрок_не_бот:
	LDA опция_режим_сложность
	AND #$03
...
; 2 таблицы, вероятно для высоты возможности делать супер
; байты сравниваются с координатой Z lo
; первые 3 для игрока, вторые 3 для бота
; зависит от сложности игры
table_04_BDD7_высота_удара_с_ноги:		; 60fps корректировка высоты для возможности супера (еще не скорректировано)
.byte $10,$1C,$1D,		$24,$1C,$14
;.byte $10,$1C,$24,		$24,$1C,$14

table_04_BDDD_высота_удара_головой:		; 60fps корректировка высоты для возможности супера (еще не скорректировано)
.byte $20,$32,$35,		$38,$32,$2C
