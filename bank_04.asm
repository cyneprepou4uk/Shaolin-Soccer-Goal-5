...
; bzk 022 из одной таблицы BD4A сделано 2
	...
	STA разновидность_супера
	TAY
	LDA table_04_BD4A_скорость_X_Y,Y		; 60фпс возможно, работает и так, но может лучше переделать под уменьшение в 3 раза
	BEQ @пропуск
	STA $2D
	LDA #$00
	STA $2C
...
@пропуск:
	...
	ASL
	TAY
	LDA table_04_BD4A_скорость_Z + 1,Y		; 60fps скорость Z	+++
	STA скорость_Z_hi,X
...
_loc_04_BC62_записать_таймер_супера:
	...
	LDA $2C
;	ASL		; 60fps таймер мяча, вроде помогло удвоение	+++
@запись_таймера:
...
table_04_BD4A_скорость_Z:	; 60fps скорость Z	+++
.byte $08,$00
.byte $08,$F8
.byte $08,$01
.byte $10,$04
...
; обычный удар, красный ближний, красный дальний, ...
