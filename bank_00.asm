...
bra_00_8780:
	...
	TXA		; не используется, удалить (пока нельзя, таблица звуков не доделана)
	ASL
	ASL
	TAY
	LDA #$81
...
bra_00_88B9:
	LDA $01
_loc_00_88BB:		; сюда много переходов
	STA $07C1,Y
	CPX #$04
	BCS bra_00_88CC
	LDA $0719,X
...
table_00_B05F_B0E9:
	LDA #$00
	STA $0605
	LDA $28		; в этом адресе всегда 00, удалить проверку
	BEQ bra_00_B0F5
	JMP _loc_00_B023
bra_00_B0F5:
	RTS
...
_loc_00_BB13:
	JSR _b07_0004_запись_FF_в_номера_управляемых
	LDX #$00
	LDY #$00
	LDA опция_режим_сложность
	AND #$F0
	BEQ @запись_управляемых
@цикл:
	BMI @запись_управляемых
	INY
	INY
	INY
	INY
	...
	JMP @цикл
@запись_управляемых:
	LDA table_00_BDB9_управляемые,Y
	STA номер_управляемого,X
	STA копия_номера_управляемого,X
	INY
	INX
	CPX количество_игроков
	BCC @запись_управляемых
	JSR _b07_0002_корректировка_байтов_управляемых_до_игры
	LDY #$08
	STY банк_спрайтов
	INY
	STY банк_спрайтов + 3
...
@продолжить_2:
	...
	LDX #$0E
bra_00_BBBE:
	LDA table_00_BDE3_номерДвижения,Y
