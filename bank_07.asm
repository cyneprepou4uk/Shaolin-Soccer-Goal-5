...
.export _loc_07_C059
_loc_07_C059:
	JMP _поставить_флаг_увеличения_яркости
.export _loc_07_C05C
_loc_07_C05C:
	JMP _loc_07_D073
...
_loc_07_C06B:
	JMP _loc_07_EFAD
.export _b03_яркость_палитры_и_запись_в_буфер
_b03_яркость_палитры_и_запись_в_буфер:
	JMP _яркость_палитры_и_запись_в_буфер
.export _loc_07_C071
_loc_07_C071:
	JMP _loc_07_C317
...
bra_07_C5C9:
	...
	JSR _яркость_палитры_и_запись_в_буфер
	LDA #$00
	STA $0692
	LDA #$3F
	STA байт_2006_hi_палитра
...
_loc_07_C735:
	LDA #$00
	STA таймер_катсцены
	JSR _поставить_флаг_увеличения_яркости
	INC тип_экрана
	SEI
	JMP _loc_07_C698
...
.export _loc_07_CEBD
_loc_07_CEBD:		; кадры анимации игрока исходя из его позы
	ASL
	TAY
	LDA ($30),Y
	STA $2C
...
_поставить_флаг_увеличения_яркости:
	LDA #$00
	STA флаг_яркости
...
; D0D5
_яркость_палитры_и_запись_в_буфер:
...
@пропуск_палитры_спрайтов:
	LDA счетчик_буфера_палитры
	BEQ bra_07_D1B6		; прыжок на RTS в ебеня, сделать поближе
	LDA буфер_палитры
	LDY #$04
@цикл:
	STA буфер_палитры,Y
...
bra_07_D1B1:
	LDA #$00
	STA a: дополнительная_задержка
bra_07_D1B6:		; прыжок из ебеней, удалить
	RTS
...
_loc_07_ECA9:
	...
	LDA #$00
	STA разновидность_NMI
	LDA байт_для_2000		; тут лучше сделать jsr на _включить_NMI
	ORA #$80
	STA байт_для_2000
	STA $2000
	LDA #$0F
...
_включить_рендеринг:
	LDA байт_для_2001
	ORA #$18
	BNE _запись_в_2001	; возможно лучше продублировать тут код по ссылке, изза имени ссылки
...
; тут какой-то мусорный код, на случай, если после переключения банка
; каким-то образом он не переключился на тот, который нужно было
.byte $EA,$AE,$FF,$BF,$80,$EA

; LDX 00 не был использован, ссылок не найдено, удалить
	LDX #$00
_обнуление_ZP_с_X_до_F8:
	LDA #$00
@цикл:
	STA $00,X
	INX
...
_обработчик_NMI:
	...
	BMI @80_или_C0
	BVC _вид_NMI_00
	BVS _вид_NMI_80	; BVS не используется, удалить
@80_или_C0:
	BVS @C0
	JMP _вид_NMI_80
@C0:
