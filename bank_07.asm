...
_b07_D036_изменение_яркости_на_логотипе:
	...
	STA скорость_яркости
	JSR _b07_яркость_палитры_и_запись_в_буфер
	JSR _loc_07_D187_изменение_яркости_на_логотипе
	RTS
...
_loc_07_D61A_рисовка_информации_статусбара:
	LDA подтип_экрана
...
@продолжить:
	...
	JMP _loc_07_D67C_рожи_в_статусбаре_справа
@рожи_в_статусбаре_слева:
	...
	STA $2006
	LDA #$81
	JSR _loc_07_D6CF_записать_из_буфера_в_видеопамять
	LDA #$07
	JSR _loc_07_D6CF_записать_из_буфера_в_видеопамять
	LDA #$27
	JSR _loc_07_D6CF_записать_из_буфера_в_видеопамять
	...
	LDA #$00
	STA $2006
_loc_07_D67C_рожи_в_статусбаре_справа:
	LDA флаг_обновления_тайлов_экрана
	AND #$10
	BNE @обновить_рожи
	JMP _loc_07_D6C6_очистить_флаг
@обновить_рожи:
	LDA $2002
	LDA #$2A
	STA $2006
	...
	LDA #$00
	STA $2006
_loc_07_D6C6_очистить_флаг:
	LDA флаг_обновления_тайлов_экрана
	AND #$CF
	STA флаг_обновления_тайлов_экрана
	RTS

_loc_07_D6CF_записать_из_буфера_в_видеопамять:
...
@увеличить_loss:
	JSR _loc_07_D7A4_увеличить_время_loss
...
_loc_07_D7A4_увеличить_время_loss:
	INC loss_терции
	LDA loss_терции
...
bra_07_E95D:		; прыжок на RTS можно переместить повыше, взяв другой RTS, а этот удалить. проверить что не нужен
	RTS
...
bra_07_EFCC:
	...
	BEQ _loc_07_EFE5_запись_в_видеопамять_трех_буферов
	JMP _loc_07_C244
bra_07_EFD4:	; 60fps !!! удалено поочередное выполнение этих трех подпрограмм на трех разных кадрах
	LDA #$01
	STA $401C
	JSR _loc_07_F0B4_рисовка_поля_при_скроллинге
	JSR _loc_07_D61A_рисовка_информации_статусбара
	JSR _loc_07_F340
	LDA #$00
	STA $401C
	RTS
...
_loc_07_F0B4_рисовка_поля_при_скроллинге:
	...
	BEQ @гола_не_было
	JSR _loc_07_F166_изменить_тайлы_сетки_ворот_после_гола
...
_loc_07_F166_изменить_тайлы_сетки_ворот_после_гола:
; bzk 024 переписан код
.SCOPE
temp = $48		; хранение X
	LDA байт_для_2000
...
_loc_07_F340:
	...
	CMP #$FF
	BEQ @буфера_сейчас_записывать_не_требуется
	JMP _loc_07_EFE5_запись_в_видеопамять_трех_буферов
@буфера_сейчас_записывать_не_требуется:
	LDA подтип_экрана
	CMP копия_подтипа_экрана
	BNE bra_07_F3A4___RTS
	CMP #$02
	BEQ @игра_на_поле_включая_ауты
	CMP #$06
	BEQ @игра_на_поле_включая_ауты
	JMP _loc_07_F382_отрисовка_следов
...
_loc_07_F382_отрисовка_следов:
	LDX счетчик_следов
	BEQ bra_07_F3A4___RTS
	DEX
...
bra_07_FD0F:
	STA $E000
	LDX #$00
@мусорный_цикл_для_IRQ_1:
	DEX
	BPL @мусорный_цикл_для_IRQ_1
	LDA $2002
	LDX #$00
	STX $2005
	STX $2005
...
_loc_07_FD4C:
	STA $E000
	LDX #$00
@мусорный_цикл_для_IRQ_2:
	DEX
	BPL @мусорный_цикл_для_IRQ_2
	LDA $2002
	LDX #$00
	STX $2005
	STX $2005
